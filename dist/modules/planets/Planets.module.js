"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PlanetsModule = void 0;
const common_1 = require("@nestjs/common");
const Planets_controller_1 = require("./Planets.controller");
const Planets_service_1 = require("./Planets.service");
const Planets_repository_1 = require("./Planets.repository");
const typeorm_1 = require("@nestjs/typeorm");
const Planets_entity_1 = require("../../entities/Planets.entity");
let PlanetsModule = class PlanetsModule {
    constructor(planetRepository) {
        this.planetRepository = planetRepository;
    }
    async onModuleInit() {
        console.log('preloading planets');
        await this.planetRepository.seedPlanets();
    }
};
exports.PlanetsModule = PlanetsModule;
exports.PlanetsModule = PlanetsModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([Planets_entity_1.PlanetEntity])],
        controllers: [Planets_controller_1.PlanetsController],
        providers: [Planets_service_1.PlanetsService, Planets_repository_1.PlanetsRepository],
    }),
    __metadata("design:paramtypes", [Planets_repository_1.PlanetsRepository])
], PlanetsModule);
//# sourceMappingURL=Planets.module.js.map